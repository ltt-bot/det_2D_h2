...
heights = np.array([np.zeros((len_ar,2)),np.zeros((len_ar,2)),np.zeros((len_ar,2))])
# vel_3 = np.zeros((int((end_timestep - start_timestep) / 1000),2))
file_names = ['/scratch/gpfs/MUELLER/ltt/Hydrogen/2D/plt/60_farther/plt_h' ,
             '/scratch/gpfs/MUELLER/ltt/Methane/2D/case2/60/plt_h',
             '/scratch/gpfs/MUELLER/ltt/Methane/2D/60_fine/120_inlet_fineplt_2D']
while (start_timestep < end_timestep[0]):
    for i in range(3):
        if start_timestep < end_timestep[i]:
            ...
            mdot_ch4 = 0
            if i > 0:
                ch4 = np.array(first["Y(CH4)"])
                mdot_ch4 =  np.trapezoid((dens[:,0,0] * ch4[:,0,0] * y_vel[:,0,0]), dx=dx_)
            ...
            isp = thrust/((mdot_h2 + mdot_ch4) * 980.665)
            ...
            #height = np.unravel_index(np.where(temp_1[max_index1:max_index1+(5 + (i*2)),:,0] < 300),temp_1[max_index1:max_index1+(5 + (i*2)),:,0].shape)[1]
            height = np.max(np.unravel_index(np.where(temp_1[:,:60,0]< 300),temp_1[:,:60,0].shape)[1]) * 0.025
            ...
            heights[i,int((start_timestep - start) / diff),:] = [f_1.current_time, height]


        start_timestep = start_timestep + diff

vars = [vel,mdots,thrusts,isps,fisps,heights]
names = ["Velocity", "mdot","thrust","isp","fsp","height"]
y_title = ["Velocity (m/s)", "ṁ (kg/ms)", "Thrust (kN/m)", "I$_{sp}$ (sec)","F$_{sp} ($\frac{kN s}{kg m}$", "Detonation Wave Height (cm)"]

for i,val in zip(range(len(vars)),vars):

    f, (ax,axs) = plt.subplots(1,2, figsize=(7,4), width_ratios=[1,0.75])

    ax.plot(val[0,:,0] * 10**3,val[0,:,1], label="Case 1", color=colors[0],lw=1.5)
    ax.plot(val[1,:,0]* 10**3,val[1,:,1], label="Case 2", color=colors[1],lw=1.5)
    ax.plot(val[2,:,0]* 10**3,val[2,:,1], label="Case 3", color=colors[2],lw=1.5)
    ax.grid()
    ax.legend()
    ...
    plt.savefig("methane_fig/" + names[i] + "_fuel.png", dpi=1200)
    plt.close()
