...
fisps = np.zeros((int((end_timestep - start_timestep) / diff),2))
heights = np.zeros((int((end_timestep - start_timestep) / diff),2))
while (start_timestep < end_timestep):
    #file_name = '/scratch/gpfs/MUELLER/ltt/Hydrogen/2D/plt/60_farther/plt_h' 
    file_name = '/scratch/gpfs/MUELLER/ltt/Hydrogen/2D/plt/60_rk/plt_h'
    ...
    mdot = np.trapezoid((dens[:,199,0] * y_vel[:,199,0]), dx=0.025) * 10**(-1)
    thrust = np.trapezoid(dens[:,199,0] * (y_vel[:,199,0]**2) + pres_1[:,199,0] - 1013250, dx=0.025)
    mdot_h2 = np.trapezoid((dens[:,0,0] * h2[:,0,0] * y_vel[:,0,0]), dx=0.025)
    mdot_air = np.trapezoid((dens[:,0,0] * (n2[:,0,0] + o2[:,0,0]) * y_vel[:,0,0]), dx=0.025)
    ...
    # if start_timestep == 20000:
    #     grad_pres = np.gradient(pres_1[:,:,0],0.025*10**(-2),0.025*10**(-2))
    #     dp_dx = grad_pres[1]
    #     dp_dy = grad_pres[0]
    ...
    #     plt.xlabel("X (cm)")
    #     plt.ylabel("Y (cm)")
    #     plt.savefig(f"pres_grad2.png", dpi=1200)
    #     plt.close()
    ...
    #     plt.savefig(f"temp_2.png", dpi=1200)
    #     plt.close()
    ...
    height = np.max(np.where(temp_1[:,:100,0]<300)[1]) * 0.025
    # print(np.max(np.where(temp_1[:,:60,0]<300)[1]))
    ...
    fisps[int((start_timestep - start) / diff),:] = [f_1.current_time, fisp]
    heights[int((start_timestep - start) / diff),:] = [f_1.current_time, height]
    start_timestep = start_timestep + diff


f, (ax,axs) = plt.subplots(1,2, figsize=(7,4), width_ratios=[1,0.75])
ax.plot(vel[:,0] * 10**3,vel[:,1],  color="#1D4DED")
ax.grid()
...
plt.savefig(f"single/Velocity_sing.png", dpi=1200)
plt.close()
...
axs.set_xticks([y + 1 for y in range(1)],labels=[""])
plt.ylabel("ṁ (kg/ms)")
plt.savefig(f"single/mdot.png", dpi=1200)
plt.close()

x_m = plt.figure()
plt.plot(thrusts[stab:,0]  * 10**(3) ,thrusts[stab:,1] * 10**(-6),  color="#1D4DED")
plt.grid()
plt.xlabel("Time (ms)")
plt.ylabel("Thrust")
plt.savefig(f"single/thrust.png", dpi=1200)
plt.close()

x_m = plt.figure()
plt.plot(heights[stab:,0]  * 10**(3) ,heights[stab:,1],  color="#1D4DED")
plt.grid()
plt.xlabel("Time (ms)")
plt.ylabel("Detonation Wave Height (cm)")
plt.savefig(f"single/height.png", dpi=1200)
plt.close()

print(np.mean(vel[stab:,1]))
print(np.mean(mdots[stab:,1]))
print(np.mean(thrusts[stab:,1]) * 10**(-6))
print(np.mean(isps[stab:,1]))
print(np.mean(fisps[stab:,1]))
print(np.mean(heights[stab:,1]))